 ...
 <!-- Social Buttons CSS -->
	...
	?>
	   <tr class="odd">
			...
			<td><i><?php echo $row['nama_instansi']; ?></i><br> <?php echo $row['jabatan']; ?>  </td>  
			<td><?php echo $row['level']; ?></td>
			<td class="center"><?php echo $tanggal; ?></td>
			<td>
			<?php if ($row['blokir']=='Y') {?>
				<a href="<?php echo base_url(); ?>users/nonaktif/<?php echo $row['username'];?>" class="btn btn-info btn-circle btn-lg"><i class="fa fa-check"></i> </a>
			<?php } else {?>
				<a href="<?php echo base_url(); ?>users/aktif/<?php echo $row['username'];?>" 	class="btn btn-warning btn-circle btn-lg"><i class="fa fa-times"></i> </a>
			<?php } ?> 
			</td>
			<td class="center">
				<a href="<?php echo base_url(); ?>users/edit/<?php echo $row['username']."/".$judul."/";?>" class="btn btn-block btn-social btn-dropbox">
...
<?php echo form_open_multipart('users/add'); ?>	 
	...
	<div style="border:1px solid #ec8585;background:#cc3333;color:#fff;border-radius:5px; padding:20px;margin-bottom:20px;">
		...
		<select class="form-control"  name="instansi" id="instansi" required>
			...
			<?php
			$dataa = $this->M_dataadmin->pilihinstansi(); 
			foreach($dataa->result_array() as $raw) {
			?> 
			<option  value="<?php echo $raw['id_instansi'];?>"><?php echo $raw['nama_instansi'];?></option>
		<?php } ?>			
		</select>
		<br><span style="color:#d5ec14;font-style:italic;"><?php echo form_error('instansi'); ?> </span> 
		</div>
	...
	<div style="border:1px solid #ec8585;background:#cc3333;color:#fff;border-radius:5px; padding:20px;margin-bottom:20px;">
		...
		<select class="form-control"  name="level" id="level"> 
			...
			echo "<option value='' selected>- Level Pengguna -</option>";
				...
				if ($raw['level']==$r['namalevel']){ ?>
				<option  value="<?php echo $r['namalevel'];?>" selected><?php echo $r['namalevel'];?></option>
				<?php } else {?>
				<option  value="<?php echo $r['namalevel'];?>"><?php echo $r['namalevel'];?></option>
				<?php }
				}
			?>
           </select>  
		...
		<div class="form-group" style="width:260px;">
		<label>Instansi <?php echo $raw['id_instansi']; ?></label>  
		<select class="form-control"  name="instansi" id="instansi"> 
			<?php 
			$dataa = $this->M_dataadmin->pilihinstansi(); 
			if ($raw['id_instansi']==0){
				echo "<option value='' selected>- Pilih -</option>";
			}   
			foreach($dataa->result_array() as $raaw) { 
			if ($raw['id_instansi']==$raaw['id_instansi']){ ?>
				<option  value="<?php echo $raaw['id_instansi'];?>" selected><?php echo $raaw['nama_instansi'];?></option>
				<?php } else {?>
				<option  value="<?php echo $raaw['id_instansi'];?>"><?php echo $raaw['nama_instansi'];?></option>
				<?php }
				}
			?>
           </select>  
